...
## == Thai == ##
resource "cloudflare_pages_project" "thai" {
  account_id        = data.sops_file.cloudflare-secret.data["cloudflare.account_id"]
  name              = "thai-amrik-co-uk"
  production_branch = "main"

  build_config {
    build_command   = ""
    root_dir        = "ITP1/Pronunciation"
    destination_dir = "app"
  }
...
resource "cloudflare_pages_domain" "thai_subdomain" {
  account_id   = data.sops_file.cloudflare-secret.data["cloudflare.account_id"]
  project_name = "thai-amrik-co-uk"
  domain       = "thai.amrik.co.uk"
}

resource "cloudflare_record" "thai_amrik_co_uk" {
  zone_id = data.sops_file.cloudflare-secret.data["cloudflare.amrik.co.uk.zone_id"]
  name    = "thai"
  type    = "CNAME"
  content = "thai-amrik-co-uk.pages.dev"
}
